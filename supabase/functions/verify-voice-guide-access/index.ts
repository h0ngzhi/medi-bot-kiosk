...
function generateToken(secret: string): string {
  ...
  return btoa(`voice_guide:${timestamp}:${btoa(data).slice(0, 16)}`);
}
...
serve(async (req) => {
  ...
  try {
    ...
    const correctPassword = Deno.env.get('VOICE_GUIDE_PASSWORD');
    
    if (!correctPassword) {
      console.error('VOICE_GUIDE_PASSWORD environment variable not set');
      return new Response(
    ...
    console.error('Error in verify-voice-guide-access:', error);
    return new Response(
